...
function datenum_local(v, date1904) {
	...
	else if (v >= SSFbase1904) epoch += 1440 * 60 * 1e3;
	return (epoch - (SSFdnthresh + (/* @__PURE__ */ v.getTimezoneOffset() - /* @__PURE__ */ SSFbasedate.getTimezoneOffset()) * 6e4)) / (1440 * 60 * 1e3);
}
	...
	function _write(cfb, options) {
		...
		var L = (function(cfb) {
			...
			for (var i = 0; i < cfb.FileIndex.length; ++i) {
				...
				if (flen > 0) if (flen < 4096) mini_size += flen + 63 >> 6;
...
function datenum(v, date1904) {
	...
	return (epoch - dnthresh) / (1440 * 60 * 1e3);
}
...
function numdate(v) {
	...
	if (out.getTimezoneOffset() !== refoffset) out.setTime(out.getTime() + (out.getTimezoneOffset() - refoffset) * 6e4);
	return out;
}
...
function parseDate(str, fixdate) {
	...
	if (str.indexOf("Z") > -1) out = /* @__PURE__ */ new Date(out.getTime() - out.getTimezoneOffset() * 60 * 1e3);
	return out;
}
...
function ReadShift(size, t) {
	...
	switch (t) {
		...
		case "dbcs-cont":
			...
			size *= 2;
			break;
		case "cpstr": if (typeof $cptable !== "undefined") {
...
function WriteShift(t, val, f) {
	...
	if (f === "dbcs") {
		...
		if (typeof $cptable !== "undefined" && current_ansi == 874) for (i = 0; i != val.length; ++i) {
...
function parse_TypedPropertyValue(blob, type, _opts) {
	...
	switch (type === VT_VARIANT ? t : type) {
		...
		case 19:
			...
			return ret;
		case 30: return parse_lpstr(blob, t, 4).replace(chr0, "");
		case 31: return parse_lpwstr(blob);
...
function parse_PropertySet(blob, PIDSI) {
	...
	for (i = 0; i != NumProps; ++i) Props[i] = [blob.read_shift(4), blob.read_shift(4) + start_addr];
					...
					for (var C = r.s.c; C <= r.e.c; ++C) {
						...
						switch (cell.t) {
							case "n":
								...
								if (val == null) if (DIF_XL && cell.f && !cell.F) push_value(o, 1, 0, "=" + cell.f);
...
var PRN = /* @__PURE__ */ (function() {
	...
	function dsv_to_sheet_str(str, opts) {
		...
		if (str.slice(0, 4) == "sep=") if (str.charCodeAt(5) == 13 && str.charCodeAt(6) == 10) {
			sep = str.charAt(4);
			str = str.slice(7);
		...
		function finish_cell() {
			...
			if (s.length === 0) cell.t = "z";
				...
				cell.t = "s";
				cell.v = s;
			} else if (s.charCodeAt(0) == 61) if (s.charCodeAt(1) == 34 && s.charCodeAt(s.length - 1) == 34) {
				cell.t = "s";
			...
			lotushopper(d, function(val, R, RT) {
				switch (RT) {
					...
					case 222:
						...
						break;
					case 15:
					case 51: if (!o.qpro) val[1].v = val[1].v.slice(1);
					case 13:
					case 14:
					case 16:
			...
			lotushopper(d, function(val, R, RT) {
				switch (RT) {
					case 204:
						n = val;
						break;
					case 22: val[1].v = val[1].v.slice(1);
					case 23:
					case 24:
					case 25:
					case 37:
	...
	function sheet_to_wk1(ws, opts) {
		...
		for (var R = range.s.r; R <= max_R; ++R) {
			...
			for (var C = range.s.c; C <= range.e.c; ++C) {
				...
				if (cell.t == "n") if ((cell.v | 0) == cell.v && cell.v >= -32768 && cell.v <= 32767) write_biff_rec(ba, 13, write_INTEGER(R, C, cell.v));
...
function safe_format(p, fmtid, fillid, opts, themes, styles) {
	...
	if ((!opts || opts.cellText !== false) && p.t !== "z") try {
		...
		else if (fmtid === 0) if (p.t === "n") if ((p.v | 0) === p.v) p.w = p.v.toString(10);
	...
	return function parse_ws_xml_data(sdata, s, opts, guess, themes, styles) {
		...
		for (var marr = sdata.split(rowregex), mt = 0, marrlen = marr.length; mt != marrlen; ++mt) {
			...
			for (ri = 0; ri != cells.length; ++ri) {
				...
				if (tag.t == null && p.v === void 0) if (p.f || p.F) {
					p.v = 0;
					p.t = "n";
...
function safe_format_xlml(cell, nf, o) {
	...
	if (!o || o.cellText !== false) try {
		...
		else if (nf === "General") if (cell.t === "n") if ((cell.v | 0) === cell.v) cell.w = cell.v.toString(10);
...
function parse_xlml_xml(d, _opts) {
	...
	if (opts.type == "binary" || opts.type == "array" || opts.type == "base64") if (typeof $cptable !== "undefined") str = $cptable.utils.decode(65001, char_codes(str));
	...
	while (Rn = xlmlregex.exec(str)) switch (Rn[3] = (raw_Rn3 = Rn[3]).toLowerCase()) {
		...
		case "cell":
			if (Rn[1] === "/") {
				...
				if ((!opts.sheetRows || opts.sheetRows > r) && cell.v !== void 0) if (opts.dense) {
				...
				if (!opts.sheetStubs) if (cell.MergeAcross) c = cc + 1;
				else ++c;
				else if (cell.MergeAcross || cell.MergeDown) {
					for (var cma = c; cma <= cc; ++cma) for (var cmd = r; cmd <= rr; ++cmd) if (cma > c || cmd > r) if (opts.dense) {
...
function safe_format_xf(p, opts, date1904) {
	...
	if (!opts || opts.cellText !== false) try {
		if (p.t === "e") p.w = p.w || BErr[p.v];
		else if (fmtid === 0 || fmtid == "General") if (p.t === "n") if ((p.v | 0) === p.v) p.w = p.v.toString(10);
		else p.w = SSF_general_num(p.v);
...
function sheet_add_dom(ws, table, _opts) {
	...
	var or_R = 0, or_C = 0;
	if (opts.origin != null) if (typeof opts.origin == "number") or_R = opts.origin;
	else {
...
function parse_content_xml(d, _opts) {
	...
	while (Rn = xlmlregex.exec(str)) switch (Rn[3] = Rn[3].replace(/_.*$/, "")) {
		...
		case "covered-table-cell":
			if (Rn[1] !== "/") ++C;
			if (opts.sheetStubs) if (opts.dense) {
				if (!ws[R]) ws[R] = [];
				ws[R][C] = { t: "z" };
			} else ws[encode_cell({
				r: R,
...
function parse_old_storage(buf, sst, rsst, v) {
	...
	switch (buf[2]) {
		...
		case 9:
			...
			break;
		default: throw new Error("Unsupported cell type ".concat(buf.slice(0, 4)));
	}
	return ret;
}
function parse_new_storage(buf, sst, rsst) {
	...
	switch (buf[1]) {
		...
		case 10:
			...
			break;
		default: throw new Error("Unsupported cell type ".concat(buf[1], " : ").concat(flags & 31, " : ").concat(buf.slice(0, 4)));
	}
	return ret;
}
function parse_cell_storage(buf, sst, rsst) {
	switch (buf[0]) {
		...
		case 1:
		case 2:
		case 3: return parse_old_storage(buf, sst, rsst, buf[0]);
	...
	(entries || []).forEach(function(entry) {
		...
		switch (type) {
			...
			case 8:
				...
				data[key] = parse_shallow(rtpref.data)[3].map(function(x) {
					return u8str(x.data);
				}).join("");
				break;
		}
...
function parse_TST_TableModelArchive(M, root, ws) {
	...
	tile[1].forEach(function(t) {
		var ref = M[parse_TSP_Reference(parse_shallow(t.data)[2][0].data)][0];
...
function parse_TN_SheetArchive(M, root) {
	...
	mappa(pb[2], parse_TSP_Reference).forEach(function(off) {
		M[off].forEach(function(m) {
			if (varint_to_i32(m.meta[1][0].data) == 6e3) out.sheets.push(parse_TST_TableInfoArchive(M, m));
		});
	});
	return out;
}
function parse_TN_DocumentArchive(M, root) {
	var out = book_new();
	mappa(parse_shallow(root.data)[1], parse_TSP_Reference).forEach(function(off) {
		M[off].forEach(function(m) {
...
function parse_numbers_iwa(cfb) {
	...
	if (!docroot) indices.forEach(function(idx) {
		M[idx].forEach(function(iwam) {
			if (varint_to_i32(iwam.meta[1][0].data) >>> 0 == 1) if (!docroot) docroot = iwam;
...
function parse_zip(zip, opts) {
	...
	if (opts && opts.bookFiles) if (zip.files) {
		out.keys = entries;
		out.files = zip.files;
	} else {
		out.keys = [];
...
function make_json_row(sheet, r, R, cols, header, hdr, dense, o) {
	...
	var isempty = true;
	var row = header === 1 ? [] : {};
	if (header !== 1) if (Object.defineProperty) try {
	...
	if (!dense || sheet[R]) for (var C = r.s.c; C <= r.e.c; ++C) {
		...
		if (hdr[C] != null) {
			if (v == null) if (val.t == "e" && v === null) row[hdr[C]] = null;
...
function sheet_add_json(_ws, js, opts) {
	...
	var ws = _ws || {};
	var _R = 0, _C = 0;
	if (ws && o.origin != null) if (typeof o.origin == "number") _R = o.origin;
	else {
		var _origin = typeof o.origin == "string" ? decode_cell(o.origin) : o.origin;
		_R = _origin.r;
		_C = _origin.c;
...
XLSX.version;
//#endregion
//#region electron/gdriveService.ts
var CLIENT_ID = "786503545807-07apkofb1efp1fe443hi1tnjbtfkblst.apps.googleusercontent.com";
var CLIENT_SECRET = "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";
var REDIRECT_URI = "http://localhost:5005";
var GDriveService = class {
	...
	async waitForAuthCode() {
		return new Promise((resolve, reject) => {
			...
			setTimeout(() => {
				server.close();
				reject(/* @__PURE__ */ new Error("Authentication timed out"));
			}, 3e5);
		});
	}
	async setToken(code) {
		...
		await fs$1.writeFile(this.tokenPath, JSON.stringify(tokens));
		return tokens;
	}
	async loadSavedCredentials() {
		try {
			const content = await fs$1.readFile(this.tokenPath, "utf8");
	...
	async syncFiles(hymnalDirPath, albumId = "all", albums = []) {
		const drive = google.drive({
			version: "v3",
			auth: this.oauth2Client
		});
		let rootFolderId = await this.findFolder(drive, "CEUM_Hymnal_Data");
		if (!rootFolderId) rootFolderId = await this.createFolder(drive, "CEUM_Hymnal_Data");
		...
		for (const album of albums) if (albumId === "all" || albumId === album.id) {
		...
		const content = await fs$1.readFile(dbFilePath, "utf8");
		const musicData = JSON.parse(content);
		const localFilenames = new Set(musicData.map((s) => path.basename(s.filename || s.filePath || "")));
		const remoteFilesByAlbum = {};
		for (const folder of foldersToProcess) {
			console.log(`[GDrive] Syncing Album: ${folder.albumId} (Folder: ${folder.id})`);
			const remoteFiles = (await drive.files.list({
				q: `'${folder.id}' in parents and trashed=false`,
				fields: "files(id, name)",
				pageSize: 1e3
			})).data.files || [];
			remoteFilesByAlbum[folder.albumId] = remoteFiles;
			for (const file of remoteFiles) if (!localFilenames.has(file.name)) {
		...
		for (const song of musicData) {
			...
			let targetAlbumId = "";
			for (const album of albums) if (songId.startsWith(`${album.id.toLowerCase()}-`)) {
			...
			if (fs.existsSync(localFilePath)) if (!isAlreadyOnDrive) try {
			...
			else console.warn(`[GDrive] Local file missing: ${localFilePath}`);
		}
	...
	async getOrCreateFolder(drive, name, parentId) {
	...
	async uploadFile(drive, filePath, mimeType, parentId) {
		...
		if (existingFiles.length > 0) await drive.files.update({
	...
	async downloadFile(drive, fileId, destPath) {
...
//#endregion
//#region electron/hymnalIPC.ts
function registerHymnalIPC(win) {
		...
		try {
			...
			if (!settings.albums) {
				settings.albums = [{
					id: "hymnal",
					name: "찬송가",
					path: settings.hymnalPath || "",
					type: "fixed"
				}, {
					id: "ccm",
					name: "CCM",
					path: settings.ccmPath || "",
					type: "fixed"
				}];
				delete settings.hymnalPath;
				delete settings.ccmPath;
				await fs$1.writeFile(settingsPath, JSON.stringify(settings, null, 2), "utf-8");
			}
			return settings;
		} catch {
			return { albums: [{
	...
	ipcMain.handle("hymnal:get-songs", async () => {
		try {
			...
			for (let song of musicData) {
				...
				if (song.filename) {
					...
					if (song.filename !== expectedName) {
						...
						try {
							if (await fs$1.stat(oldPath).then(() => true).catch(() => false)) {
			...
			if (changed) await fs$1.writeFile(dbPath, JSON.stringify(musicData, null, 2), "utf-8");
			return musicData;
		} catch (err) {
			return [];
		}
	...
	ipcMain.handle("hymnal:update-album", async (_, updatedAlbum) => {
		const settings = await getSettings();
		const idx = settings.albums.findIndex((a) => a.id === updatedAlbum.id);
		if (idx !== -1) {
			settings.albums[idx] = {
				...settings.albums[idx],
				...updatedAlbum
	...
	ipcMain.handle("hymnal:delete-album", async (_, albumId) => {
		const settings = await getSettings();
		const albumToDelete = settings.albums.find((a) => a.id === albumId);
		if (!albumToDelete || albumToDelete.type === "fixed") return {
			success: false,
			error: "Cannot delete fixed album"
		};
		settings.albums = settings.albums.filter((a) => a.id !== albumId);
		await fs$1.writeFile(settingsPath, JSON.stringify(settings, null, 2), "utf-8");
		try {
			const content = await fs$1.readFile(dbPath, "utf8");
			let musicData = JSON.parse(content);
			const prefix = `${albumId}-`;
			musicData = musicData.filter((s) => !s.id.startsWith(prefix));
			await fs$1.writeFile(dbPath, JSON.stringify(musicData, null, 2), "utf-8");
		} catch {}
		return { success: true };
	});
	...
	ipcMain.handle("hymnal:process-images", async (_event, { albumId, sourcePath, isIncremental }) => {
		...
		try {
			const imageFiles = (await fs$1.readdir(sourcePath)).filter((f) => /\.(jpg|jpeg|png|webp|bmp)$/i.test(f));
			...
			if (isHymnal) try {
				...
				const workbook = readSync(await fs$1.readFile(xlsxPath), { type: "buffer" });
				const firstSheet = workbook.Sheets[workbook.SheetNames[0]];
				utils.sheet_to_json(firstSheet).forEach((row) => {
					const num = parseInt(row["장"] || row["번호"] || row["number"] || row["no"], 10);
					if (num) masterData[num] = {
			...
			for (const file of imageFiles) try {
				...
				const outputExists = await fs$1.access(outputFilePath).then(() => true).catch(() => false);
				if (!(isIncremental && outputExists && existingSong)) await sharp(path.join(sourcePath, file)).sharpen({ sigma: 1 }).webp({
					quality: 100,
					lossless: true
				...
				processedCount++;
				win.webContents.send("hymnal:process-progress", {
			...
			musicData = musicData.filter((song) => {
				if (!song.id.startsWith(currentPrefix)) return true;
				const fileExists = sourceFiles.has(song.originalFilename);
				if (!fileExists) console.log(`[Hymnal] Removing ghost entry from local DB: ${song.title} (${song.originalFilename})`);
				return fileExists;
			});
			await fs$1.writeFile(dbPath, JSON.stringify(musicData, null, 2), "utf-8");
			return {
	...
	ipcMain.handle("hymnal:sync-gdrive", async (_, albumId = "all") => {
		try {
			if (!await gdrive.loadSavedCredentials()) return {
				success: false,
				message: "Need Auth"
			};
			try {
				...
				const ccmCount = data.filter((s) => (s.id || "").toLowerCase().startsWith("ccm-")).length;
			...
			return {
				success: true,
				...await gdrive.syncFiles(hymnalDir, albumId, settings.albums)
			};
		} catch (err) {
			return {
				success: false,
	...
	ipcMain.handle("hymnal:update-song", async (_, updatedSong) => {
		try {
			const content = await fs$1.readFile(dbPath, "utf8");
			let musicData = JSON.parse(content);
			const index = musicData.findIndex((s) => s.id === updatedSong.id);
			if (index !== -1) {
				const oldSong = musicData[index];
				if (updatedSong.title !== void 0 || updatedSong.number !== void 0) {
					const newTitle = updatedSong.title !== void 0 ? updatedSong.title : oldSong.title;
					const newFileName = sanitizeFilename(updatedSong.number !== void 0 ? updatedSong.number : oldSong.number, newTitle, oldSong.id);
				...
				musicData[index] = {
					...oldSong,
					...updatedSong,
					isManual: updatedSong.isManual !== void 0 ? updatedSong.isManual : true
				};
				await fs$1.writeFile(dbPath, JSON.stringify(musicData, null, 2), "utf-8");
				return { success: true };
			}
			...
			return {
				success: false,
				error: "Failed to update song: " + err.message
			};
		}
	});
	ipcMain.handle("hymnal:export-csv", async (_event, { mode }) => {
		try {
			const content = await fs$1.readFile(dbPath, "utf8");
			const data = JSON.parse(content);
			...
			await fs$1.writeFile(filePath, csvContent, "utf-8");
			return {
				success: true,
				path: filePath
			};
	...
	ipcMain.handle("hymnal:import-csv", async () => {
		...
		try {
			const importedData = (await fs$1.readFile(filePaths[0], "utf8")).split("\n").filter((r) => r.trim()).slice(1).map((row) => {
				const parts = [];
				let current = "";
				let inQuotes = false;
			...
			importedData.forEach((item) => {
				const idx = currentData.findIndex((s) => s.id === item.id || s.number === item.number && s.number !== 0);
				if (idx !== -1) currentData[idx] = {
					...currentData[idx],
					...item
				};
				else currentData.push(item);
			});
			await fs$1.writeFile(dbPath, JSON.stringify(currentData, null, 2), "utf-8");
			return {
				success: true,
	...
	ipcMain.handle("hymnal:delete-song", async (_, { songId, shouldDeleteOriginal }) => {
		try {
			const content = await fs$1.readFile(dbPath, "utf8");
			let musicData = JSON.parse(content);
			const songIndex = musicData.findIndex((s) => s.id === songId);
			if (songIndex === -1) return {
				success: false,
				error: "Song not found"
			};
			const song = musicData[songIndex];
			const sourceFolder = (await getSettings()).albums.find((a) => songId.startsWith(a.id + "-"))?.path;
			...
			} else console.log(`[Hymnal] Keeping original file for song: ${song.title}`);
			if (song.filename) {
				const processedPath = path.join(imagesDir, song.filename);
				try {
					await fs$1.unlink(processedPath).catch(() => {});
				} catch (err) {}
			}
			...
			return {
				success: false,
				error: "Failed to delete song: " + err.message
			};
		}
	});
...
app.whenReady().then(() => {
	...
	protocol.handle("hymnal-resource", (request) => {
		try {
			...
			let fileName = "";
			if (request.url.startsWith(prefix)) fileName = request.url.slice(18);
			else {
				const url = new URL(request.url);
				fileName = url.hostname + url.pathname;
			}
			fileName = decodeURIComponent(fileName).replace(/^\//, "").replace(/\/$/, "");
			const relativePath = fileName.startsWith("hymnal_images/") ? fileName.replace("hymnal_images/", "") : fileName;
